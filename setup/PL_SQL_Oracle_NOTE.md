...
#You can also specify only certain permissions to user
```
...
ALTER TABLE child_table ADD constraint t_id_pk PRIMARY KEY(id)
ALTER TABLE child_table ADD constraint t_id_fk FOREIGN KEY(id) REFERENCES parent_table(id);

--convention to add constraint names with <table>_<key>_<type> 
```
...
-- we can also use some without grouping if we want to find them for whole table 
```
...
--JOINs all table info from t1 (even those whose value for 
--the condition is null) but leave the ones who are null in t2
```
...
--JOINs all table info from t2 (even those whose value for 
--the condition is null) but leave the ones who are null in t1
```
...
--JOINs all table info from t1 and t2 (even those whose value for the condition is null) 
```
...
--perform a CROSS or Cartesian multiplication for the tables and produce a table with the output
```
...
-- % -> any no of words
-- _ -> one word
-- '%xyz' returns all words that end with xyz
-- '%xyz%' returns all words where there's xyz in the middle
-- 'xyz%' returns all words that start with xyz
-- 'xyz_' returns words that end with xyz and any one letter afterwards
```
...
```sql
CREATE TABLE t(
    c1 NUMBER NOT NULL, c2 NUMBER NOT NULL, c3 VARCHAR2(100),
...
CREATE TABLE t1(
    ...
    c2 NUMBER,
    CONSTRAINT FOREIGN KEY (c2) REFERENCES t2(c2)
);
```
...
| `NUMBER x`       | Integer (-2147­483648 to 214748­3647)                         |
