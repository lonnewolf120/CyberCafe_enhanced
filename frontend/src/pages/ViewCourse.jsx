...
export default function ViewCourse() {
  ...
  useEffect(() => {
    ; (async () => {
      const courseData = await getFullDetailsOfCourse(COURSE_ID, token)
      console.log("Course Data here... ", courseData)
      dispatch(setCourseSectionData(courseData.courseDetails[0].sections))
      dispatch(setEntireCourseData(courseData.courseDetails[0]))
      dispatch(setCompletedLectures(courseData.completedVideos))
      let lectures = 0
      courseData?.courseDetails[0]?.sections?.forEach((sec) => {
