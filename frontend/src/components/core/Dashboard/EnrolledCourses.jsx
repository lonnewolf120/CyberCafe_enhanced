...
export default function EnrolledCourses() {
  ...
  const getEnrolledCourses = async () => {
    try {
      ...
      //FIXME: add the enrolled courses properly
      // res -> [{course: [course1, course2, ...]}]
    ...
    <>
      <div className="text-4xl text-richblack-5 font-boogaloo text-center sm:text-left">Enrolled Courses</div>
      {
        <div className="my-8 text-richblack-5">
          ...
          {console.log("EnrolledCourses (from EnrolledCourses.jsx): ", enrolledCourses)}
          {/* Course Names */}
          {
            enrolledCourses?.map((theCourse, i, arr) => (
              console.log(`enrolledCourse ${i}`,theCourse?.course[0]),
              <div
              ...
              >
                <div
                  ...
                  onClick={() => {
                    navigate(
                      `/view-course/${theCourse?.course[0].COURSE_ID}/section/${theCourse?.course[0].sections[0]?.SECTION_ID}/sub-section/${theCourse?.course[0]?.sections[0]?.SUBSECTION_ID}`
                    )
                  }}
                >
                  <Img
                    src={theCourse?.course[0]?.THUMBNAIL}
                    alt="course_img"
                    className="h-14 w-14 rounded-lg object-cover"
                  />

                  <div className="flex max-w-xs flex-col gap-2">
                    <p className="font-semibold">{theCourse?.course[0]?.COURSE_NAME}</p>
                    <p className="text-xs text-richblack-300">
                      {theCourse?.course[0]?.COURSE_DESCRIPTION?.length > 50
                        ? `${theCourse?.course[0]?.COURSE_DESCRIPTION.slice(0, 50)}...`
                        : theCourse?.course[0]?.COURSE_DESCRIPTION}
                    </p>
                  </div>
                </div>
                ...
                <div className='sm:hidden'>
                  <div className=" px-2 py-3">{theCourse?.course[0]?.totalDuration}</div>

                  <div className="flex sm:w-2/5 flex-col gap-2 px-2 py-3">
                    {/* {console.log('Course ============== ', course.progressPercentage)} */}

                    <p>Progress: {theCourse?.course[0].progressPercentage || 0}%</p>
                    <ProgressBar
                      completed={theCourse?.course[0].progressPercentage || 0}
                      height="8px"
                ...
                {/* duration -  progress */}
                <div className="hidden w-1/5 sm:flex px-2 py-3">{theCourse?.course[0]?.totalDuration}</div>
