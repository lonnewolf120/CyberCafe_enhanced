...
export default function CoursesTable({ courses, setCourses, loading, setLoading }) {
  ...
  // delete course
  const handleCourseDelete = async (COURSE_ID) => {
    setLoading(true)
    const toastId = toast.loading('Deleting...');
    await deleteCourse({ COURSE_ID: COURSE_ID }, token)
                ...
                >
                  <Td className="flex flex-1 gap-x-4 relative">
                    ...
                    <div className="flex flex-col">
                      <p className="text-lg font-semibold text-richblack-5 capitalize">{course.COURSE_NAME}</p>
                      <p className="text-xs text-richblack-300 ">
                        {/* {course.COURSE_DESCRIPTION?.split(" ").length > TRUNCATE_LENGTH
                  ...
                  <Td className="text-sm font-medium text-richblack-100 ">
                    ...
                    <button
                      disabled={loading}
                      onClick={() => { navigate(`/dashboard/edit-course/${course.COURSE_ID}`) }}
                      title="Edit"
                    ...
                    <button
                      ...
                      onClick={() => {
                        setConfirmationModal({
                          ...
                          btn2Text: "Cancel",
                          btn1Handler: !loading
                            ? () => handleCourseDelete(course.COURSE_ID)
                            : () => { },
                          btn2Handler: !loading
                            ? () => setConfirmationModal(null)
                            : () => { },
